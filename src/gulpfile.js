    ...
    fs.readdirSync('client/app/js/'+componentType+'s').forEach(function(filename){
      if (filename.match(/\.js$/)) {
        ...
        switch(componentType){
          ...
          case 'filter': filter=function(name){return name.substr(0,1).toUpperCase()+name.replace(/\.js$/,'').substr(1)+'Filter'};  break;
          case 'directive':
            filter=function(name){
              var result=[];
              name=name.replace(/\.js$/,'').split('-');
              result.push(name.shift());
              name.forEach(function(w){
                result.push(w.substr(0,1).toUpperCase()+w.substr(1));
              });
              return result.join('');
            }
            break;
